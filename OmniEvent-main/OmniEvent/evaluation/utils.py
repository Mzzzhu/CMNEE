...
def dump_preds_eae(trainer: Union[Trainer, Seq2SeqTrainer],
               tokenizer: PreTrainedTokenizer,
               data_class: type,
               output_dir: Union[str,Path],
               model_args: ModelArguments,
    ...
    if model_args.paradigm == "token_classification":
        pred_labels = [data_args.id2role[pred] for pred in preds]
    elif model_args.paradigm == "sequence_labeling":
        pred_labels = get_ace2005_argument_extraction_sl(preds, labels, data_file, data_args, dataset.is_overflow)
    elif model_args.paradigm == "mrc":
        pred_labels = get_ace2005_argument_extraction_mrc(preds, labels, data_file, data_args, dataset.is_overflow)
    elif model_args.paradigm == "seq2seq":
        pred_labels = get_ace2005_argument_extraction_s2s(preds, labels, data_file, data_args, None)
    else:
        raise NotImplementedError
    ...
    json.dump(pred_labels, open(save_path, "w", encoding='utf-8'), ensure_ascii=False)
    logger.info("EAE {} preds dumped to {}\n EAE finished!".format(mode, save_path))
