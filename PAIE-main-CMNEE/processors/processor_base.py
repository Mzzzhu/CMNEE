...
class DSET_processor:
    ...
    def split_chinese_string(self, sentence):
        result = []
    ...
    def convert_offset(self, raw_s, raw_e, offset_add, space_flag_list, delete_flag):
    ...
    def _read_roles(self, role_path):
        ...
        with open(role_path, "r", encoding='gb18030') as f:
    ...
    def _create_example_cmnee(self, lines):
        W = self.args.window_size
        assert (W % 2 == 0)
        all_args_num = 0
        ...
        for line in lines:
            ...
            for event in events:
                ...
                cut_text = full_text
                event_trigger = {}
                event_trigger["start"] = event['trigger']["offset"][0]
                event_trigger["end"] = event['trigger']["offset"][1]
                ...
                if sent_length > W + 1:
                    if event_trigger['end'] <= W // 2:  # trigger word is located at the front of the sents
                        ...
                        'start'] >= sent_length - W / 2:  # trigger word is located at the latter of the sents
                        offset = sent_length - (W + 1)
                        min_s += offset
                        max_e += offset
                        event_trigger['start'] -= offset
                ...
                for arg_info in event['arguments']:
                    all_args_num += 1
                    evt_arg = dict()
                    evt_arg['start'] = arg_info['offset'][0]
                    evt_arg['end'] = arg_info['offset'][1]
                    evt_arg['role'] = arg_info['role']
                    evt_arg['text'] = arg_info['text']
                    if evt_arg['start'] < min_s or evt_arg['end'] > max_e:
                        self.invalid_arg_num += 1
                ...
                examples.append(
                    # Event(doc_key, None, cut_text, event_type, event_trigger, event_args, full_text, first_word_locs))
                    Event(doc_key, None, cut_text, event_type, event_trigger, event_args, full_text))
    ...
    def create_example(self, file_path):
        ...
        elif self.args.dataset_type=='cmnee':
            lines = self._read_jsonlines(file_path)
            return self._create_example_cmnee(lines)
        else:
    ...
    def generate_dataloader(self, set_type):
        ...
        features = self.convert_examples_to_features(self.args, examples)
        ...
        if self.collate_fn:
            dataloader = DataLoader(dataset, sampler=dataset_sampler, batch_size=self.args.batch_size, collate_fn=self.collate_fn, drop_last=True)
        else:
            dataloader = DataLoader(dataset, sampler=dataset_sampler, batch_size=self.args.batch_size, drop_last=True)
