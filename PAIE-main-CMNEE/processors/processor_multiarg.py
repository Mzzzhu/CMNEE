...
class MultiargProcessor(DSET_processor):
    ...
    def _read_prompt_group(prompt_path):
        with open(prompt_path, encoding='gb18030') as f:
            lines = f.readlines()
        prompts = dict()
        for line in lines:
            if not line:
    ...
    def convert_examples_to_features(self, args, examples):
        ...
        for example_idx, example in enumerate(examples):
            ...
            enc_text = ''.join(sent)
            try:
                len(sent) == len(enc_text)
            except:
                print("enc_text {} length is worng!".format(enc_text))
            # print("id", example_id)
            # print("sent",sent)
            # print("exapmle", example.full_text)
            # print("enc_txt",enc_text)
            # print("len", len(enc_text))

            # change the mapping to idx2tuple (start/end word idx)
            old_tok_to_char_index = []
            old_tok_to_char_index2 = []# old tok: split by oneie
            old_tok_to_new_tok_index = []  # new tok: split by BART 左闭右开

            curr = 0
            for tok in sent:
                if tok not in EXTERNAL_TOKENS:
                    old_tok_to_char_index.append([curr, curr+len(tok)-1])
                    old_tok_to_char_index2.append([tok, curr, curr+len(tok)-1])# exact word start char and end char index
                curr += len(tok)#中文按字

            enc = self.tokenizer(enc_text)
            enctext = self.tokenizer.tokenize(enc_text)
            enc_input_ids, enc_mask_ids = enc["input_ids"], enc["attention_mask"]
            try:
                len(enc) == len(enctext)
            except:
                print("enctext {} length is worng!".format(enctext))
            try:
                len(enc_input_ids) == len(enctext)
            except:
                print("enctext {} length is worng2!".format(enctext))
