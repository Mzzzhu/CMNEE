...
def convert_cmnee_to_unified(data_path: str,
    ...
    for line in lines:
        ...
        if len(full_text)+sum(space_flag_list)+sum(delete_flag) == len(full_text_2):
            instance = dict()
            instance["sentence"] = full_text
            instance["s_start"] = 0 #did not split to sentences
            instance["ner"] = []
            instance["relation"] = []
            instance["event"] = []
            for event_id, event in enumerate(events):
                ...
                event_trigger["start"], event_trigger["end"] = convert_offset(raw_s,raw_e,offset_add,space_flag_list,delete_flag)
                event_trigger["text"] = event['trigger']["text"]

                instance["event"].append([[event_trigger["start"], event_type]])
                try:
                    full_text[event_trigger["start"]:event_trigger["end"]] == full_text_raw[raw_s:raw_e]
                except:
                ...
                for arg_info in event['arguments']:
                    ...
                    if arg_info['offset'][0] in offset_add and arg_info['offset'][1] in offset_add:
                        ...
                        evt_arg['start'], evt_arg['end'] = convert_offset(raw_arg_s,raw_arg_e,offset_add,space_flag_list,delete_flag)
                        evt_arg['role'] = convert2chinese[arg_info['role']]
                        evt_arg['text'] = arg_info['text']

                        instance["event"][event_id].append([evt_arg['start'], evt_arg['end'], evt_arg['role']])

                        event_args.append(evt_arg)
                        try:
                            full_text[evt_arg["start"]:evt_arg["end"]] == full_text_raw[raw_s:raw_e]
                        except:
            ...
            for coref_arg_info in coref_arguments:
                ...
                for coref_arg_s in coref_arg_info:
                    ...
                    if coref_arg_s['offset'][0] in offset_add and coref_arg_s['offset'][1] in offset_add:
                        ...
                        coref_arg_start, coref_arg_end = convert_offset(raw_coref_arg_s, raw_coref_arg_e, offset_add,
                                                                        space_flag_list, delete_flag)
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--save_dir", type=str, default="../dataset/cmnee_eeqa")
