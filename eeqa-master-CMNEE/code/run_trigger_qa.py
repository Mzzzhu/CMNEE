    ...
    with open(input_file, "r", encoding='utf-8') as f:
        for line in f:
            ...
            for event in events:
                ...
                # offset, category = (event[0][0] - s_start, event[0][1] - s_start) , event[0][1]
...
def evaluate(args, eval_examples, category_vocab, model, device, eval_dataloader, pred_only=False):
    ...
    for sentence_id in pred_triggers:
        ...
        for trigger in pred_triggers[sentence_id]:
            sent_triggers_offset.append(trigger[0])
        # print("sent_triggers_offset", sent_triggers_offset)
...
def main(args):
    ...
    if args.do_train:
        train_examples, train_features = read_ace_examples(nth_query = args.nth_query, input_file=args.train_file, tokenizer=tokenizer, category_vocab=category_vocab, is_training=True)
        ...
        all_sentence_id = torch.tensor([f.sentence_id for f in train_features], dtype=torch.long)
        ...
        train_data = TensorDataset(all_sentence_id, all_input_ids, all_segment_ids, all_in_sentence, all_input_mask, all_labels)
        ...
        for lr in lrs:
            ...
            for epoch in range(int(args.num_train_epochs)):
                ...
                for step, batch in enumerate(train_batches):
                    ...
                    sentence_id, input_ids, segment_ids, in_sentence, input_mask, labels = batch
                    ...
                    if (step + 1) % eval_step == 0 or step == 0:
                        ...
                        save_model = False
                        if args.do_eval:
                            result, eval_preds = evaluate(args, eval_examples, category_vocab, model, device, eval_dataloader)
                            model.train()
                        ...
                        if save_model:
                            ...
                            result_t, train_preds = evaluate(args, train_examples, category_vocab, model, device,
                                                             train_dataloader)
                            with open(os.path.join(subdir, "train_trigger_predictions.json"), "w", encoding='utf-8') as writer:
                                for line in train_preds:
                                    writer.write(json.dumps(line, default=int, ensure_ascii=False) + "\n")
                            with open(os.path.join(subdir, "train_results.txt"), "w") as writer:
                                # for key in sorted(best_result.keys()):
                                for key in result_t:
                                    writer.write("%s = %s\n" % (key, str(result_t[key])))

                            with open(os.path.join(subdir, "eval_trigger_predictions.json"), "w", encoding='utf-8') as writer:
                            ...
                            if best_result:
                                with open(os.path.join(args.output_dir, "best_eval_results.txt"), "w") as writer:
                                    # for key in sorted(best_result.keys()):
                                    for key in best_result:
                                        writer.write("%s = %s\n" % (key, str(best_result[key])))
                                with open(os.path.join(args.model_dir, "best_eval_trigger_predictions.json"), "w", encoding='utf-8') as writer:
                                    for line in eval_preds:
                                        writer.write(json.dumps(line, default=int, ensure_ascii=False) + "\n")
    ...
    if args.do_eval:
        ...
        with open(os.path.join(args.model_dir, "test_results.txt"), "w") as writer:
            for key in result:
                writer.write("%s = %s\n" % (key, str(result[key])))
        with open(os.path.join(args.model_dir, "test_trigger_predictions.json"), "w", encoding='utf-8') as writer:
            for line in preds:
                writer.write(json.dumps(line, default=int, ensure_ascii=False) + "\n")
